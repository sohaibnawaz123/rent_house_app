// ignore_for_file: deprecated_member_use, library_private_types_in_public_api, curly_braces_in_flow_control_structures
...
class ItemScrollPhysics extends ScrollPhysics {
  ...
  double _getItem(ScrollPosition position) {
    double maxScrollItem = (position.maxScrollExtent / itemHeight!)
        .floorToDouble();
...
class _TimePickerSpinnerState extends State<TimePickerSpinner> {
  ...
  /// default settings
  TextStyle defaultHighlightTextStyle = TextStyle(
    fontSize: 32,
    color: Colors.black,
  );
  ...
  void initState() {
    ...
    hourController = ScrollController(
      initialScrollOffset: (currentSelectedHourIndex - 2) * _getItemHeight()!,
    );
    ...
    minuteController = ScrollController(
      initialScrollOffset: (currentSelectedMinuteIndex - 2) * _getItemHeight()!,
    );
    ...
    if (widget.onTimeChange != null) {
      WidgetsBinding.instance.addPostFrameCallback(
        (_) => widget.onTimeChange!(getDateTime()),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> contents = [
      Column(
        ...
        children: [
          Content(data: "H", textStyle: _getHighlightedTextStyle(), size: 28),
          SizedBox(
            width: _getItemWidth(),
      ...
      Column(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Content(data: "M", textStyle: _getHighlightedTextStyle(), size: 28),
  ...
  Widget spacer() {
    return SizedBox(width: _getSpacing(), height: _getItemHeight()! * 5);
  }
          ...
          if (scrollNotification.direction.toString() ==
            ...
            if (isLoop(max)) {
              ...
              if (segment == 0) {
                ...
                controller.jumpTo(
                  controller.offset + (max * _getItemHeight()!),
                );
              } else if (segment == 2) {
                onUpdateSelectedIndex(selectedIndex - max);
                controller.jumpTo(
